...
module.exports = function (bot, options) {
  ...
  const onoff = (options && options.onoff || 'on')
  ...
  function ban(msg, userid) {
    ...
    var user = msg.channel.guild.members.find(member => member.user.id === msg.author.id).catch(err => console.error(err));
    if (user && !user.hasPermission("ADMINISTRATOR")) {
      let names = user.username + "#" + user.discriminator
      user.ban(deleteMessagesAfterBanForPastDays).then((member) => {
        const channelsendlol1 = new Discord.RichEmbed()
          ...
          .addField("Moderator:", names)
          .addField("Reason:", 'Spamming')
          .addField("Server:", msg.guild)
        user.send({ embed: channelsendlol1 })
console.log(user)
        const okgoogle1 = new Discord.RichEmbed()//modlog
          .setColor('#ff0000') //change the color!!!
          .setTimestamp()
          .setThumbnail(user.avatarURL)
          .addField('Action:', "Ban")
          .addField('User:', names) //user.username + '#' + user.discriminator
          .addField("User ID:", user.id)
          .addField("Moderator:", msg.author.username + "#" + msg.author.discriminator)
          .addField("Reason:", 'Spamming')
        msg.channel.send({ embed: okgoogle1 })
        msg.guild.channels.find("name", "modlog").send({ embed: okgoogle1 }).catch();
        return true;
      }).catch((err) => {
        console.log(err)
